...
public class Main {
    public static void main(String[] args) {
        ...
        // Ordenar los países por número de ciudades en orden descendente
        List<Pais> paisesOrdenadosPorCiudades = paises.stream()
            .sorted(Comparator.comparingInt(p -> -p.getCiudades().size()))
            .collect(Collectors.toList());

        System.out.println("Países ordenados por número de ciudades (descendente):");
        paisesOrdenadosPorCiudades.forEach(pais -> 
            System.out.println("- " + pais.getNombrePais() + ": " + pais.getCiudades().size() + " ciudades"));
    }
}
